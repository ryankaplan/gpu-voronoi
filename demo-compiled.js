(function() {
  ...
  function CanvasDrawController(canvas) {
    ...
    in_HTMLElement.addEventListener4(self._canvas, 'mousedown', function(e) {
      ...
      e.stopPropagation();
      self._draw(new Vector(e.offsetX, e.offsetY));
    });
  ...
  CanvasDrawController.prototype._draw = function(point) {
    ...
    in_CanvasRenderingContext2D.setFillStyle(this._ctx, in_List.get(paintColors, this.colorIndex).toCSS());
    this._ctx.fill();
  ...
  function PhotoDemoController(canvas) {
    ...
    self._seedCtx = null;
    self._sourceImg = null;
    self._sourcePattern = null;
    self._isMouseDown = false;
    self.radius = 1;
    self.onDraw = null;
    self._canvas = canvas;
    ...
    self._sourceImg = document.getElementById('eye');
    self._sourcePattern = self._seedCtx.createPattern(self._sourceImg, 'repeat');

    // Start out by drawing the source image in the canvas displayed
    // to the user
    self._ctx.drawImage(self._sourceImg, 0, 0);
    in_HTMLElement.addEventListener4(self._canvas, 'mousedown', function(e) {
      self._isMouseDown = true;
  ...
  PhotoDemoController.prototype._draw = function(point) {
    this._ctx.beginPath();
    this._ctx.rect(point.x - this.radius | 0, point.y - this.radius | 0, __imul(this.radius, 2), __imul(this.radius, 2));
    in_CanvasRenderingContext2D.setFillStyle(this._ctx, '#FFFFFF');
    this._ctx.fill();

    // Pin the sourceImage pixels down onto the seed canvas
    // where the user is drawing
    this._seedCtx.beginPath();
    this._seedCtx.rect(point.x - this.radius | 0, point.y - this.radius | 0, __imul(this.radius, 2), __imul(this.radius, 2));
    var img = document.getElementById('eye');
    var pattern = this._seedCtx.createPattern(img, 'repeat');
    in_CanvasRenderingContext2D.setFillStyle1(this._seedCtx, pattern);
    this._seedCtx.fill();
